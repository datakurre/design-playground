...
parseHex input =
    ...
    if not allHex then
        ...
        let
            c1 = String.slice 0 1 withoutHash
            c2 = String.slice 1 2 withoutHash
            c3 = String.slice 2 3 withoutHash
...
parse6 hexStr =
    let
        rStr = String.slice 0 2 hexStr
        gStr = String.slice 2 4 hexStr
        bStr = String.slice 4 6 hexStr
    in
    Maybe.map3 Rgb
        (hexToInt rStr)
        (hexToInt gStr)
    ...
    let
        charToInt c =
            ...
            if code >= 48 && code <= 57 then
                ...
                0 -- should be prevented by allHex check
    in
